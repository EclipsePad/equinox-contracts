...
pub struct Instantiate<'a> {
    ...
    pub timelock_config: Vec<(u64, u64)>,
...
pub fn instantiate() -> Instantiate<'static> {
    ...
    assert_eq!(
        ...
        Config {
            ...
            timelock_contract: Addr::unchecked(suite.timelock_staking_contract())
        }
    );
    return Instantiate {
...
fn update_config() {
    ...
    let test_config = UpdateConfigMsg {
        ...
        timelock_contract: Some(Addr::unchecked("test").to_string()),
    };
    ...
    assert_eq!(
        ...
        Config {
            ...
            timelock_contract: Addr::unchecked("test"),
        }
    );
}
...
fn allow_users() {
    ...
    let mut suite = instantiate_data.suite;
    assert_eq!(
        suite.query_flexible_is_allowed(suite.lockdrop()).unwrap(),
        true
    );
    let err = suite
        .update_flexible_allow_users(ALICE, vec![Addr::unchecked("test").to_string()])
        .unwrap_err();
    assert_eq!(
        ContractError::Admin(AdminError::NotAdmin {}),
        err.downcast().unwrap()
    ...
    suite
        .update_flexible_allow_users(&suite.admin(), vec![Addr::unchecked("test").to_string()])
        .unwrap();
    assert_eq!(
        suite.query_flexible_is_allowed("test".to_string()).unwrap(),
        true
    );
    let err = suite
        .update_flexible_block_users(ALICE, vec![Addr::unchecked("test").to_string()])
        .unwrap_err();
    assert_eq!(
        ContractError::Admin(AdminError::NotAdmin {}),
        err.downcast().unwrap()
    ...
    suite
        .update_flexible_block_users(&suite.admin(), vec![Addr::unchecked("test").to_string()])
        .unwrap();
    assert_eq!(
        suite.query_flexible_is_allowed("test".to_string()).unwrap(),
        false
...
fn single_stake() {
    ...
    let flexible_stake_reward = FlexibleReward {
        eclip: Uint128::zero(),
    ...
    let amount = 100_000;
    suite
        .mint_native(suite.reward_distributor_contract(), suite.eclip(), amount)
        .unwrap();
    suite
        .send_astro(
            astro_staking_initiator,
    ...
    assert_eq!(
        suite.query_astro_staking_total_shares().unwrap(),
        1001000u128
    );
    assert_eq!(
...
fn stake() {
    ...
    suite.flexible_unstake(BOB, 1_000, None).unwrap();
    ...
    suite.flexible_unstake(BOB, 2_000, None).unwrap();
    assert_eq!(suite.query_flexible_staking(BOB).unwrap(), 4_000);
    assert_eq!(suite.query_total_flexible_staking().unwrap(), 6_000);
...
fn claim() {
    ...
    suite.flexible_unstake(ALICE, 1_000, None).unwrap();

    assert_eq!(suite.query_eclipastro_balance(ALICE).unwrap(), 1024);
}

#[test]
fn relock_without_deposit() {
    let instantiate_data = instantiate();
    let mut suite = instantiate_data.suite;
    let astro_staking_initiator = instantiate_data.astro_staking_initiator;
    ...
    suite
        ...
        .unwrap();
    suite
        .flexible_relock(ALICE, 86400 * 30, Some(Uint128::from(100u128)), None)
        .unwrap();
    assert_eq!(suite.query_flexible_staking(ALICE).unwrap(), 0u128);
    let timelock_total_staking = suite.query_timelock_total_staking().unwrap();
    assert_eq!(timelock_total_staking, 1000u128);
    let timelock_staking = suite.query_timelock_staking(ALICE).unwrap();
    assert_eq!(timelock_staking[0].duration, 86400 * 30);
    assert_eq!(timelock_staking[0].staking[0].amount.u128(), 1000u128);
    assert_eq!(timelock_staking[0].staking[0].locked_at, suite.get_time());
}
...
fn relock_with_deposit() {
    ...
    suite
        .stake_astro(astro_staking_initiator, astro_amount)
        .unwrap();
    let alice_flexible_stake_amount = 1500;
    suite
        .convert_astro(ALICE, alice_flexible_stake_amount)
        .unwrap();
    suite.flexible_stake(ALICE, 1000).unwrap();
    suite
        .flexible_relock_with_deposit(ALICE, 500, 86400 * 30, Some(Uint128::from(100u128)), None)
        .unwrap();
    assert_eq!(suite.query_flexible_staking(ALICE).unwrap(), 0u128);
    let timelock_total_staking = suite.query_timelock_total_staking().unwrap();
    assert_eq!(timelock_total_staking, 1500u128);
    let timelock_staking = suite.query_timelock_staking(ALICE).unwrap();
    assert_eq!(timelock_staking[0].duration, 86400 * 30);
    assert_eq!(timelock_staking[0].staking[0].amount.u128(), 1500u128);
    assert_eq!(timelock_staking[0].staking[0].locked_at, suite.get_time());
}
