...
fn single_sided_incentives_distribution() {
    ...
    let alice_eclip_balance = suite
        .query_balance_native(ALICE.to_string(), suite.eclip())
        .unwrap();
    assert_eq!(alice_beclip_balance - prev_alice_beclip_balance, 11111); // 100%
    assert_eq!(alice_eclip_balance - prev_alice_eclip_balance, 11111); // 100%
    let user_info = suite.query_user_single_lockup_info(ALICE).unwrap();
    assert_eq!(
        user_info
            .iter()
    ...
    let alice_eclip_balance = suite
        .query_balance_native(ALICE.to_string(), suite.eclip())
        .unwrap();
    assert_eq!(alice_beclip_balance - prev_alice_beclip_balance, 33333); // 100%
    assert_eq!(alice_eclip_balance - prev_alice_eclip_balance, 33333); // 100%
    let user_info = suite.query_user_single_lockup_info(ALICE).unwrap();
    assert_eq!(
        user_info
            .iter()
...
fn lp_incentives_distribution() {
    ...
    assert_eq!(alice_beclip_balance - prev_alice_beclip_balance, 699997); // 100%
    let user_info = suite.query_user_lp_lockup_info(ALICE).unwrap();
    assert_eq!(
        user_info
            .iter()
...
fn restake_and_unlock() {
    ...
    // assert_eq!(suite.query_user_single_lockup_info(ALICE).unwrap(), vec![]);
    suite.single_lockup_relock(ALICE, 0, 2592000).unwrap();
    let alice_beclip_balance = suite.query_beclip_balance(ALICE).unwrap();
    assert_eq!(alice_beclip_balance - prev_alice_beclip_balance, 11111); // flexible incentives
    ...
    let alice_beclip_balance = suite.query_beclip_balance(ALICE).unwrap();
    assert_eq!(alice_beclip_balance - prev_alice_beclip_balance, 22222);
    let alice_eclip_balance = suite
        .query_balance_native(ALICE.to_string(), suite.eclip())
        .unwrap();
    assert_eq!(alice_eclip_balance - prev_alice_eclip_balance, 22222);
    let alice_info = suite.query_user_single_lockup_info(ALICE).unwrap();
    assert_eq!(
        alice_info
            .iter()
