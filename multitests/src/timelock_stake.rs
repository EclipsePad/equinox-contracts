...
fn stake() {
    ...
    let err = suite.timelock_stake(ALICE, 1_000, 10, None).unwrap_err();
    assert_eq!(
        ContractError::NoLockingPeriodFound(10),
        err.downcast().unwrap()
    );
    suite.timelock_stake(ALICE, 1_000, ONE_MONTH, None).unwrap();
    assert_eq!(
    ...
    suite.convert_astro(ALICE, 1_000).unwrap();
    suite
        .timelock_stake(ALICE, 1_000, THREE_MONTH, None)
        .unwrap();
    ...
    suite.timelock_stake(BOB, 1_000, ONE_MONTH, None).unwrap();
    assert_eq!(
    ...
    suite
        .timelock_unstake(ALICE, THREE_MONTH, time - THREE_MONTH, None, None)
        .unwrap();
    alice_staking = vec![
        UserStaking {
            duration: ONE_MONTH,
    ...
    suite
        .timelock_restake(ALICE, ONE_MONTH, time - THREE_MONTH, SIX_MONTH, None, None)
        .unwrap();
    alice_staking = vec![
        UserStaking {
            duration: THREE_MONTH,
...
fn claim() {
    ...
    suite.timelock_stake(BOB, 7_000, THREE_MONTH, None).unwrap();
    ...
    suite
        .timelock_stake(ALICE, 4_000, THREE_MONTH, None)
        .unwrap();
    ...
    suite
        .timelock_unstake(ALICE, ONE_MONTH, time - 43200, None, None)
        .unwrap();

    assert_eq!(suite.query_eclipastro_balance(ALICE).unwrap(), 2970);
}
