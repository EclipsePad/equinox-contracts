...
const CONTRACT_NAME: &str = "crates.io:cw20-gift";
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateWhitelist {
            senders,
            recipients,
        } => execute_update_whitelist(deps, env, info, senders, recipients),
    }
}
...
pub fn execute_transfer(
    ...
    check_whitelist(deps.storage, &info.sender, &rcpt_addr)?;
...
pub fn execute_update_whitelist(
    ...
    let mut whitelist = WHITELIST.load(deps.storage).unwrap_or_default();

    if let Some(x) = senders {
        whitelist.senders = x
            .into_iter()
    ...
    if let Some(x) = recipients {
        whitelist.recipients = x
            .into_iter()
            .map(|x| deps.api.addr_validate(&x))
            .collect::<StdResult<Vec<Addr>>>()?;
    }
...
pub fn check_whitelist(
    ...
    let whitelist = WHITELIST.load(storage).unwrap_or_default();

    if whitelist.senders.contains(sender) || whitelist.recipients.contains(recipient) {
        return Ok(());
    }
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::QueryWhitelist {} => to_json_binary(&query_whitelist(deps)?),
    }
}
...
pub fn query_whitelist(deps: Deps) -> StdResult<Whitelist> {
    Ok(WHITELIST.load(deps.storage).unwrap_or_default())
}
