...
use eclipse_base::{
    ...
    staking::{
        state::{
            ...
            PAGINATION_CONFIG, STAKER_INFO, TOTAL_LOCKING_ESSENCE,
            TOTAL_STAKING_ESSENCE_COMPONENTS, VAULTS_LIMIT,
        },
        types::{Config, LockerInfo, PaginationConfig, StakerInfo},
    },
};
...
pub fn accumulate_rewards(
    ...
    pagination_config.staking_pagination_index = stakers.last().map(|(index, _)| index.to_owned());
    pagination_config.locking_pagination_index = lockers.last().map(|(index, _)| index.to_owned());
    ...
    for (staker_address, mut staker_info) in stakers {
        staker_info.vaults = staker_info
            ...
            .map(|mut vault| {
                ...
                vault.accumulated_rewards = math::calc_staking_rewards_per_vault(
            ...
            .map(|mut locker_info| {
                ...
                locker_info.vaults = locker_info
                    ...
                    .map(|mut vault| {
                        ...
                        vault.accumulated_rewards = math::calc_locking_rewards_per_vault(
                            vault.accumulated_rewards,
