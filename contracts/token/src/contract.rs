...
fn verify_xml_preamble(data: &[u8]) -> Result<(), CW20ContractError> {
    ...
    let preamble = data
        ...
        .next()
        .ok_or(CW20ContractError::InvalidXmlPreamble {})?;

    const PREFIX: &[u8] = b"<?xml ";
    const POSTFIX: &[u8] = b"?>";

    if !(preamble.starts_with(PREFIX) && preamble.ends_with(POSTFIX)) {
        Err(CW20ContractError::InvalidXmlPreamble {})
    } else {
        Ok(())
    }
...
/// Validates XML logo
fn verify_xml_logo(logo: &[u8]) -> Result<(), CW20ContractError> {
    verify_xml_preamble(logo)?;

    if logo.len() > LOGO_SIZE_CAP {
        Err(CW20ContractError::LogoTooBig {})
    } else {
        Ok(())
    }
}
...
/// Validates png logo
fn verify_png_logo(logo: &[u8]) -> Result<(), CW20ContractError> {
    // PNG header format:
    ...
    if logo.len() > LOGO_SIZE_CAP {
        ...
        Err(CW20ContractError::InvalidPngHeader {})
    } else {
        Ok(())
    }
}
...
fn verify_logo(logo: &Logo) -> Result<(), CW20ContractError> {
    match logo {
...
mod tests {
    ...
    mod marketing {
        ...
        fn png() {
            ...
            let instantiate_msg = InstantiateMsg {
                name: "eclipastro Token".to_string(),
                symbol: "eclipastro".to_string(),
                decimals: 6,
                initial_balances: vec![],
                mint: None,
                marketing: Some(InstantiateMarketingInfo {
