...
use astroport::{
    ...
    pair::{ExecuteMsg as PairExecuteMsg, PoolResponse, QueryMsg as PairQueryMsg},
    staking::{ExecuteMsg as StakingExecuteMsg, QueryMsg as StakingQueryMsg},
};
use cosmwasm_std::{
...
use equinox_msg::token_converter::ExecuteMsg as ConverterExecuteMsg;
...
const MINT_AMOUNT: u128 = 100_000_000_000u128;
...
pub fn try_claim(deps: DepsMut, env: Env, info: MessageInfo) -> Result<Response, ContractError> {
    ...
    if astro_balance.amount.lt(&Uint128::from(MINIMUM_BALANCE)) {
        astro_amount_to_mint += Uint128::from(MINT_AMOUNT);
    }
    if xastro_balance.amount.lt(&Uint128::from(MINIMUM_BALANCE)) {
        astro_amount_to_mint += Uint128::from(MINT_AMOUNT);
        msg_list.push(
            WasmMsg::Execute {
